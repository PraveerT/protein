...
def fps_with_indices(data, number):
    fps_idx = pointnet2_utils.furthest_point_sample(data, number)
    fps_data = pointnet2_utils.gather_operation(data.transpose(1, 2).contiguous(), fps_idx).transpose(1, 2).contiguous()
...
def gather_points(points, indices):
    B, N, C = points.shape
    B_ind, G, S = indices.shape
    
    # Đảm bảo batch size khớp nhau
    assert B == B_ind, "Batch size of points and indices must be the same."

    indices_base = torch.arange(B, device=points.device).view(B, 1, 1) * N
    indices_flat = indices + indices_base
    
    indices_flat = indices_flat.view(-1)
    points_flat = points.view(B * N, C)
    
    gathered_flat = points_flat[indices_flat]
    gathered_points = gathered_flat.view(B, G, S, C)
    
    return gathered_points


def gather_points_1d(data, order):
    B, G, C = data.shape
    B_ord, G_ord = order.shape
    
    assert B == B_ord and G == G_ord, "Batch and Group size of data and order must be the same."


    batch_indices = torch.arange(B, device=data.device).view(B, 1)
    shuffled_data = data[batch_indices, order, :]
