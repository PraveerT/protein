...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            points, mask, target = data
            points = points.cuda()
            mask = mask.cuda()
            target = target.cuda()

            coords = points[:, :, :3].contiguous()      # Shape: [B, N, 3]
            features = points[:, :, 3].long() # Shape: [B, N]
            ...
            # points = train_transforms(points)
            ret = base_model(coords, features, mask)
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger=None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            features = points[:, :, 3].long()

            #points = misc.fps(points, npoints)

            logits = base_model(coords, features, mask)
        ...
        cm = confusion_matrix(test_label_numpy, test_pred_numpy, labels=np.arange(num_classes))
        
        acc_per_class = cm.diagonal() / (cm.sum(axis=1) + 1e-6) * 100.0
        
        macc = np.mean(acc_per_class)
        
        print_log('[Validation] EPOCH: %d  OA = %.4f  mAcc = %.4f' % (epoch, acc, macc), logger=logger)

        if hasattr(config.dataset.val._base_, 'CLASSES'):
             class_names = config.dataset.val._base_.CLASSES
        else:
             class_names = [f'Class_{i+1}' for i in range(num_classes)]
             
        print_log('Detailed Accuracy per Class:', logger=logger)
        for i, class_acc in enumerate(acc_per_class):
            print_log(f'  - {class_names[i]:<20}: {class_acc:.4f}', logger=logger)
...
def validate_vote(base_model, test_dataloader, epoch, val_writer, args, config, logger=None, times=10):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            coords_raw = points[:, :, :3].contiguous()
            features_raw = points[:, :, 3].long()

            local_pred = []
            for _ in range(times):
                coords_augmented = test_transforms(coords_raw.clone())   
                logits = base_model(coords_augmented, features_raw, mask)
                local_pred.append(logits.detach().unsqueeze(0))

            pred_logits = torch.cat(local_pred, dim=0).mean(0)
            _, pred_choice = torch.max(pred_logits, -1)
