...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, xyz):
        ...
        # fps the centers out
        center, center_idx = misc.fps_with_indices(xyz, self.num_group) 
        # knn to get the neighborhood
        # idx = knn_query(xyz, center, self.group_size)  # B G M
        _, group_idx = self.knn(xyz, center)  # [B, G, S]
        assert group_idx.size(1) == self.num_group
        assert group_idx.size(2) == self.group_size
        idx_base = torch.arange(0, batch_size, device=xyz.device).view(-1, 1, 1) * num_points
        idx_for_gather = group_idx + idx_base
        idx_for_gather = idx_for_gather.view(-1)
        
        neighborhood = xyz.view(batch_size * num_points, -1)[idx_for_gather, :]
        neighborhood = neighborhood.view(batch_size, self.num_group, self.group_size, 3).contiguous()
        
        # normalize
        neighborhood = neighborhood - center.unsqueeze(2)
        return neighborhood, center, group_idx, center_idx
...
class PointMambaScan(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.chem_embed_dim = 128
        self.chem_embedding = nn.Embedding(21, self.chem_embed_dim)
        self.chem_proj = nn.Linear(self.chem_embed_dim, self.trans_dim)
        self.group_divider = Group(num_group=self.num_group, group_size=self.group_size)
        self.encoder = Encoder(encoder_channel=self.encoder_dims)
        self.token_proj = nn.Linear(self.encoder_dims, self.trans_dim)
        self.chem_proj = nn.Linear(self.chem_embed_dim, self.trans_dim)
    ...
    def forward(self, coords, features, mask):
        neighborhood_coords_norm, center_coords, group_idx, center_indices = self.group_divider(coords)
        geometric_tokens = self.encoder(neighborhood_coords_norm) 
        B = coords.size(0)
        batch_indices = torch.arange(B, device=coords.device).view(B, 1)
        center_features = features[batch_indices, center_indices.long()]
        chemical_tokens = self.chem_embedding(center_features)

        geometric_tokens_proj = self.token_proj(geometric_tokens)
        chemical_tokens_proj = self.chem_proj(chemical_tokens)

        pos_embedding = self.pos_embed(center_coords)

        final_tokens_unsorted = geometric_tokens_proj + chemical_tokens_proj
        
        # Forward
        _, _, _, final_tokens_f, pos_f = serialization_func(
            p=center_coords,          # Dùng tọa độ để tính thứ tự
            x=final_tokens_unsorted,  # Yêu cầu nó sắp xếp token nội dung
            x_res=pos_embedding,      # Yêu cầu nó sắp xếp positional embedding
            order='hilbert'
        )
        
        # Backward
        _, _, _, final_tokens_b, pos_b = serialization_func(
            p=center_coords, 
            x=final_tokens_unsorted, 
            x_res=pos_embedding, 
            order='hilbert-trans'
        )
      
        final_tokens_f = apply_OrderScale(final_tokens_f, self.OrderScale_gamma_1, self.OrderScale_beta_1)
        final_tokens_b = apply_OrderScale(final_tokens_b, self.OrderScale_gamma_2, self.OrderScale_beta_2)
        
        final_tokens_content = torch.cat([final_tokens_f, final_tokens_b], dim=1)
        final_tokens_pos = torch.cat([pos_f, pos_b], dim=1)
        
        if self.use_cls_token:
            cls_token = self.cls_token.expand(final_tokens_content.size(0), -1, -1)
            cls_pos = self.cls_pos.expand(final_tokens_pos.size(0), -1, -1)
            final_tokens_content = torch.cat([final_tokens_content, cls_token], dim=1)
            final_tokens_pos = torch.cat([final_tokens_pos, cls_pos], dim=1)

        mamba_output = self.blocks(final_tokens_content, final_tokens_pos)
        # x = self.norm(x)
    
        num_tokens = final_tokens_pos.shape[1]
        token_mask = torch.ones(mamba_output.shape[0], num_tokens, device=mamba_output.device, dtype=torch.bool)
        
        concat_f_list = []
        
        if self.use_cls_token:
            cls_output = mamba_output[:, -1, :]
            concat_f_list.append(cls_output)
            token_mask[:, -1] = False

        real_tokens = mamba_output
        if self.avg_head:
            avg_pooled = torch.sum(real_tokens * token_mask.unsqueeze(-1), dim=1) / torch.sum(token_mask, dim=1, keepdim=True)
            concat_f_list.append(avg_pooled)
        if self.max_head:
            max_pooled = torch.max(real_tokens.masked_fill(~token_mask.unsqueeze(-1), -torch.finfo(real_tokens.dtype).max), dim=1)[0]
            concat_f_list.append(max_pooled)
        
        final_representation = torch.cat(concat_f_list, dim=1)
        
        ret = self.cls_head_finetune(final_representation)
        return ret
