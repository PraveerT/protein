...
class QuaternionOps:
    ...
    def compute_rotation_quaternion(v1, v2):
        ...
        default_axis = torch.tensor([[1., 0., 0.]], device=v1.device).expand(v1_flat.shape[0], -1)
    ...
    @staticmethod
    def compute_motion_consistency(pos_prev, pos_curr):
        """Compute cycle consistency error and forward rotation"""
        q_forward = QuaternionOps.compute_rotation_quaternion(pos_prev, pos_curr)
        q_backward = QuaternionOps.compute_rotation_quaternion(pos_curr, pos_prev)
        
        identity = torch.zeros_like(q_forward)
        identity[:, 0] = 1  # [1,0,0,0] for each point
        
        q_cycle = QuaternionOps.quaternion_multiply(q_forward, q_backward)
